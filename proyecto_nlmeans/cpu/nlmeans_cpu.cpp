...
void nlMeansCPU(const std::vector<float>& entrada,
...
int main(int argc, char* argv[]) {
    ...
    // Cargar con canales originales para detectar si es color o gris
    ...
    std::cout << "=== NL-Means CPU Secuencial ===" << std::endl;
    std::cout << "Imagen: " << ancho << "x" << alto << "  Canales: " << canales << (canales == 3 ? " (color)" : " (gris)") << std::endl;
    std::cout << "Patch: 7x7  Ventana: 21x21  Hilos: 1" << std::endl;
    ...
    // Agregar ruido si el usuario lo pidio
    ...
    for (int c = 0; c < canales; c++)
        nlMeansCPU(imgCanales[c], imgLimpia[c], ancho, alto, tamPatch, tamVentana, h);
    ...
    stbi_write_jpg("../resultados/imagenes/limpia_cpu_secuencial.jpg", ancho, alto, canales, salida.data(), 95);

    std::ofstream csv("../resultados/metricas/metricas_cpu_secuencial.csv");
    csv << "version,resolucion,canales,hilos,tiempo_ms,tiempo_s\n";
    csv << "CPU_Secuencial," << ancho << "x" << alto << "," << canales << ",1," << tiempoMs << "," << tiempoMs / 1000.0 << "\n";
    csv.close();

    std::cout << "Imagen guardada en resultados/imagenes/limpia_cpu_secuencial.jpg" << std::endl;
    std::cout << "Metricas guardadas en resultados/metricas/metricas_cpu_secuencial.csv" << std::endl;
    return 0;
}
